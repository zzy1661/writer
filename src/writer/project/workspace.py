...
def create_workspace(
    ...
    *,
    ...
    directories = [
        ...
        root / "备忘",
        root / "正文",
        # 伏笔目录是所有题材共有（per 2026-07-17 落地）—— 不再是
        # 玄幻专属；每本书都需要铺设伏笔与回收计划。
        root / "伏笔",
    ]
    if with_ideas_dir:
    ...
    files = {
        ...
        root / "备忘" / "待办.md": "# 待办\n\n",
        # 伏笔表是所有题材共有 —— 每本书都需要铺设伏笔，
        # 不应该藏在玄幻脚手架里。Markdown 表示与现有的
        # ``伏笔.yaml`` ledger (结构化查询) 并存,前者供人类编辑
        # 与 /伏笔 directive append,后者供 ``foreshadow_search``
        # 工具过滤查询。
        root / "伏笔" / "伏笔表.md": (
            "# 伏笔表\n\n"
            ...
            "|----|------|---------|---------|------|------|\n"
            "\n"
            "> 状态: ``laid`` (已埋伏) / ``paid`` (已回收)。\n"
            "> 章节锚点格式: ``第 X 章`` 或 ``第 X 卷 Y 章``。\n"
            "> 人类可手编;``/伏笔`` directive 自动追加新行;``foreshadow_search``\n"
            "> 工具读 ``伏笔.yaml`` ledger(配套文件)做结构化查询。\n"
        ),
    }
...
def _genre_scaffolding(
    ...
    scaffolds: dict[str, dict[Path, str]] = {
        ...
        "玄幻": {
            # 伏笔已是所有题材共有脚手架（per 2026-07-17）；玄幻专属
            # 只剩境界表。``apply_genre_scaffolding`` 对回退到玄幻的
            # 历史项目**不会**清理既有 ``伏笔/``目录（additive 语义）。
            root / "大纲" / "境界表.md": (
                "# 境界表\n\n记录修炼等级体系与各境界节点。\n"
            ),
        },
        "言情": {
            # 人设目录删除（per 2026-07-17）：男主 / 女主定位已由
            # ``人物/<角色名>.md`` 三重标签生成法承担；``人设/``
            # 双文件结构与八项遗留模型冲突,不再生成。
            root / "大纲" / "感情线时间轴.md": (
                "# 感情线时间轴\n\n按关系阶段拆章。\n"
            ),
