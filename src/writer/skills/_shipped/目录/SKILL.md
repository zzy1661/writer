...
- 项目根目录无 `大纲/大纲.md` 时 yield `SkillError("未找到大纲文件，请先执行 /大纲 <创意>")`。
- 大纲内容为空时，提示用户重新生成大纲。
- 已存在 `大纲/章节目录.md` 时提示用户将覆盖。

## 可调点

- 项目想用"分卷"结构（卷 1 / 卷 2）时，可在 章节目录.md 头部加 `## 分卷` 节。
