...
uv run writer new 我的小说       # 创建新书项目（含 .writer/ 元数据）
...
用户输入 → CLI (typer + rich + prompt_toolkit)
       ...
       L1 IntentRouter (Protocol)        ← writer/routing/
              ↓ AgentAction
       L2 Engine 状态机 (AsyncGenerator)  ← writer/engine/Engine 类
              ↓
...
`Engine.run(ctx)` 是个 `AsyncIterator[TextChunk | ActionEvent | ToolCall | ToolResult | Interrupt | Done | ErrorEvent]`,每轮产出一个 `Done` 终结(由 `Engine` 类的私有方法 `_engine_loop` + 派发 helper `_run_*` 实现)。`Done.reason` 当前覆盖八个分支:
