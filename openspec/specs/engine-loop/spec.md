# Capability: engine-loop

## Purpose

The per-turn dispatch state machine at the heart of the agent engine. The engine loop translates the `AgentAction` from intent routing into a stream of events — text chunks, action events, tool calls and results, interrupts, errors, and a terminal `Done` event — that consumers (REPL, future `EngineSession`) can render. It owns the lifecycle of one user turn; session-level state lives elsewhere.
## Requirements
