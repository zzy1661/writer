# Capability: engine-session

## Purpose

Cross-turn session state container that sits *outside* the stateless Engine Loop. `EngineSession` is constructed once at REPL startup and reused for every turn. It owns the session identity (frozen `session_id` + `started_at`), the mutable cross-turn state (`project_root`, `project_state`, `pending_interrupt`, `turns`), and the `EngineDeps` instance (built once at construction; `tool_runtime` swapped when `project_root` changes). The Engine Loop remains a stateless `AsyncGenerator` per 备忘 16 §"Engine 是无状态 AsyncGenerator"; the session is the bridge between turns.
## Requirements
...
### Requirement: Engine fixes session identity across turns
...
### Requirement: Engine holds mutable cross-turn state
...
### Requirement: Engine builds deps once at construction
...
### Requirement: Engine rebuilds tool_runtime when project_root changes
...
### Requirement: Engine records each turn via TurnRecord
...
### Requirement: Engine tracks pending Interrupt across turns
...
### Requirement: REPL owns one Engine for its lifetime
...
### Requirement: Engine package public surface
