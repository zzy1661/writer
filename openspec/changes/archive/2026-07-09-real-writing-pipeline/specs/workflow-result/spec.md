...
The structured return contract for `EngineDeps.run_workflow`. Workflows (e.g. `write_chapter`, `review_chapter`) return a `WorkflowResult` instead of an untyped `Iterable[str]` so the engine can route on `status` to pick the right `Done` reason, persist `artifacts` deterministically, and surface `metrics` to the CLI.
...
### Requirement: EngineDeps.run_workflow returns WorkflowResult
