# Capability: prose-llm

## Purpose

The LLM client contract for long-form prose generation (chapter drafts, review reports, etc.) — distinct from the existing structured-output / tool-calling paths in `writer.llm.structured` and `writer.llm.agent.LLMToolLoop`. A `LLMProseClient` is always present in `EngineDeps`; the choice between Real and Deterministic depends on whether an API key is configured, not on the call site.
...
`EngineDeps.prose_client: LLMProseClient` MUST be set by `production_deps` to either `RealProseClient(get_llm(settings))` (when `settings.has_api_key` is True) or `DeterministicProseClient(prep_context=writer.context.prep_context)` (otherwise). The field MUST NOT be `None` under any configuration.
...
#### Scenario: production_deps without API key wires DeterministicProseClient
- **WHEN** `production_deps(project_root=root)` is called with `settings.has_api_key=False`
- **THEN** `deps.prose_client.name` MUST equal `"deterministic"`
- **AND** no LLMConfigError MUST be raised during construction (the prose client is always present, unlike `tool_loop` which may be None)
