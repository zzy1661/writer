...
> **2026-07-16 重要修订**(覆盖 2026-07-14 修订,覆盖 2026-07-09 修订):本文档原版本以 `writer.runner.loop.run_runner` 为主入口 + `_DefaultRunnerDeps.run_workflow` 占位 stub 为基础。截至 2026-07-13,**`Engine` 类**(2026-07-13 引入)成为引擎主入口;截至 2026-07-16 (`chg-rename-engine-runner`),命名混淆解决:
> - **`EngineSession` → `Engine`**(长寿命跨轮主控,`src/writer/session/engine.py`)
> - **`Engine` → `Runner`**(短命单轮派发,`src/writer/runner/runner.py`)
> - `engine.loop.run_runner` **不保留 compat shim**(2026-07-16 后)— 新代码应直接持有 `Runner` 实例(典型来源 `Engine.runner`)
>
> **`Runner` 类形态**(per 2026-07-16 重命名):
> - 主类:`Runner` 持 `RunnerDeps`(DI 容器,长生命周期)+ `RunnerConfig`(per-loop 配置)
> - 公开方法:`Runner.run(ctx) -> AsyncIterator[Event]` / `Runner.replace_deps(new_deps)` / `Runner.replace_cfg(new_cfg)`
...
>
> **`Engine` 类形态**(per 2026-07-16 重命名,原 `EngineSession`):
> - 跨 turn 主控,持 `runner: Runner` 实例 + 跨 turn 状态
> - 公开方法:`run_turn(text)` / `set_project_root(root)` / `record_turn(input, reason)` / `refresh_project_state()` / `refresh_project_genre()`
> - `Engine.runner` 替代旧 `Engine.engine`,`__post_init__` 一次性构造
>
> **`RunnerDeps` 当前形态**(2026-07-16 实测):9 字段 + 5 rebind 方法 + 2 普通方法。`tool_loop` / `prose_client` / `review_llm` / `settings` 均存在;`story_agent` 字段已删(`chg-remove-roles`)。
>
> **长任务编排**:2026-07-09 `real-writing-pipeline` PR2 把 `write_chapter` 从 sync stub 升级为 LangGraph 5 节点图;PR3 把 `review_chapter` 升级为 LangGraph 5 节点图。`workflow_pending` 不再是合法 `DoneReason`(`Done(workflow_completed)` 替代)。2026-07-14 `_plan_chapter_node` LLM 驱动(strict-reject deterministic)。
>
> 本节"已落地的 Runner 层结构"(§5) 描述的是 2026-07-08 形态;2026-07-13 起 `src/writer/engine/` 多出 `engine.py` 主类文件;2026-07-16 起 `src/writer/engine/` 整个包改名为 `src/writer/runner/`(同步把 `engine.py` 改为 `runner.py`,主类 `Engine` 改为 `Runner`);`RunnerDeps` 字段已扩展到 9 个;其余概念(ReAct / Plan-and-Execute / Tool-Use / Markdown directive)仍生效。
...
| 引擎层 | `match` dispatch + AsyncGenerator | 调度 `AgentAction` 到对应分支,产出事件流 | `Runner`(per `chg-rename-engine-runner` 2026-07-16,`src/writer/runner/runner.py`)+ `RunnerDeps` |
| 工作流 / 工具 / directive 层 | LangGraph(`write_chapter` / `review_chapter` 5 节点图,PR2/PR3 2026-07-09 实装) + Tool-use + Markdown directive | 长任务、文件 IO、业务规则 | `_DefaultRunnerDeps.run_workflow` + `ToolRegistry` + `DirectiveRegistry`(3 shipped:`/大纲` `/目录` `/人物`) |
...
- **引擎**:`Runner.run(ctx)` AsyncGenerator + `match action.action_type`
  - 主范式:事件流 dispatch(单层 match,顶层 try/except 三层 except:`ToolError` / `SkillError` / `Exception`)
  - `run_command` 命中 directive 时走 `Runner._run_directive()` 把 SKILL.md body + references 喂给 LLM 工具循环
  - `call_tool` 走 `Runner._run_tool`(rule-only)/ `Runner._run_tool_loop`(LLM 多步)
  - `start_workflow` 走 `Runner._run_workflow` → `RunnerDeps.run_workflow`(`write_chapter` / `review_chapter` LangGraph 5 节点图,2026-07-09 实装)
- **工具层**:9 个 builtin Tool(`safe_read_file` / `safe_list_dir` / `safe_write_file` / `safe_edit_file` / `safe_glob` / `project_search` / `foreshadow_search` / `chapter_locate` / `wordcount`)
  - 主范式:Tool-Use + LangChain `bind_tools` / JSON-prompt 双 provider
  - 业务级"做什么"由 **Markdown SKILL.md directives** 表达(**3 个 shipped:`/大纲` `/目录` `/人物`**),不是 Python 类
...
3. **状态机只作展示。** `ProjectState` 是 S0-S5 大状态标签,服务 `/状态` 显示;命令拦截机制(`validate_command_available`)已随 `chg-remove-state-machine-enforcement`(2026-07-12)删除,不再拦截任何指令。
4. **Tool 是 Agent 的边界。** 任何文件副作用必须经过注册过的 Tool,避免 LLM 直接编辑 Markdown。
5. **业务规则是 directive。** "写大纲怎么做"、"续写章节怎么做"、"创建角色卡怎么做"等业务规则通过 SKILL.md markdown 文件描述(per `chg-markdown-skills`,2026-07-08),LLM 在工具循环里直接消费这些指令。
...
| 状态可恢复 | ReAct 历史不可序列化检查点 | Engine 维护 turn history,WorkflowStub 占位等 LangGraph checkpoint |
...
## 已落地的 Runner 层结构(2026-07-16 重命名后)

备忘 16 给出的是 4 层概念模型。本节把"前台调度层 + 后台引擎层"具体落到 `writer.runner` 包的代码形态(2026-07-16 之前是 `writer.engine`)。

### 5 文件布局(per `chg-rename-engine-runner` 2026-07-16)

```
writer/runner/                              # 原 writer/engine/
├── __init__.py    # 公共门面: 只 re-export, 不放逻辑
├── events.py      # Event 数据类层级(frozen=True, 单一基类)
├── context.py     # RunnerContext(frozen) — 单一输入契约
├── deps.py        # RunnerDeps Protocol + production_deps()
├── config.py      # RunnerConfig(frozen, "环境冰封")
├── loop.py        # run_runner() 工厂函数 + Runner._engine_loop() AsyncGenerator
└── runner.py      # Runner 主类(原 engine.py,Engine → Runner)
```
...
| `context.py` | 用户调用 | `RunnerContext` 实例 | 不可变 (frozen) | 当次 turn |
| `deps.py` | DI 边界 | `RunnerDeps` 协议 + 默认实现 | 不可变 (Protocol + dataclass) | 长生命周期 / session 级 |
| `config.py` | ctx + 运行时 | `RunnerConfig` 实例 | 不可变 (frozen) | 当次 turn |
| `loop.py` | ctx + deps + config | AsyncIterator[Event] | — | 当次 turn |

### RunnerDeps 当前形态(2026-07-08 实测)
...
class RunnerDeps(Protocol):
    ...
    def rebind_tool_runtime(self, new_runtime) -> RunnerDeps: ...
    def rebind_skill_registry(self, new_registry) -> RunnerDeps: ...  # back-compat alias
    def rebind_directive_registry(self, new_registry) -> RunnerDeps: ...
...
新增能力只能通过 `RunnerDeps` 扩展,不直接改 `loop.py`。

### Runner 是无状态 AsyncGenerator

`runner.run(ctx)`(2026-07-16 前是 `engine.run(ctx)`)返回 `AsyncIterator[Event]`,本身不持有任何会话状态。原因:

1. **流式消费友好**: REPL `async for event` 逐事件渲染,token-level 流式输出可直接接。
2. **取消成本低**: REPL Ctrl+C 可以中断 async iteration,Runner 不持有需要清理的资源。
3. **session 可任意 restart**: 重连 session 不需要重置 Runner 内存,只需重新调用 `runner.run(ctx)`。
4. **易测**: 单个 turn = 单次 AsyncGenerator 消费,test 直接 `async for` 就行。

会话级状态(`Engine`(2026-07-16 前是 `EngineSession`)的 turn history / pending_interrupt / project_root 切换 / deps 热替换)属于 `writer.session`,不在 Runner 内部。

### 与 Layer 表的对应

回头看 4 层模型,Runner 实际上跨越前台 + 后台两层:

- **前台调度层** → `Runner._engine_loop` 调 `deps.route()`,发 `ActionEvent` 后再分派
- **后台工作流层** → Runner 内 `start_workflow` 分支走 `_run_workflow` → `RunnerDeps.run_workflow`(`write_chapter` / `review_chapter` LangGraph 5 节点图,2026-07-09 PR2/PR3 实装)
- **业务工具层** → `call_tool` 分支走 `Runner._run_tool`(同步)/ `Runner._run_tool_loop`(LLM 多步)
- **directive 层** → `run_command` 命中 `deps.directive_registry.get(action.command)` 时走 `Runner._run_directive`,body + references 喂给 LLM 工具循环
- **记忆层** → 现在是 `_build_canon_block` 纯文件拼装 + `foreshadow_search` ledger 查询,**不**在 Runner 内部

## 更新后的验收标准

在原本六条之外,补充:

- Runner 包严格 5 文件布局(`runner.py` + `loop.py` + `events/context/deps/config`),新增能力(workflow / tool / interrupt / directive)只通过 `RunnerDeps` 扩展,不直接改 `Runner._engine_loop`(2026-07-16 前是 `engine.loop`)
- `RunnerContext / RunnerConfig / Event 子类 / AgentAction` 全程 frozen(`@dataclass(frozen=True)` 或 Pydantic `model_config={"frozen": True}`)
- `runner.run(ctx)` 是 `AsyncIterator[Event]`,不持有会话状态
- `RunnerDeps` 必须 `@runtime_checkable`,便于测试时 mock
- 9 字段 + 5 rebind 方法全部需要在手写 `RunnerDeps` stub 时实现(测试 fakes 经常漏 `tool_loop = None` / `prose_client = None` / `review_llm = None` / `settings = None` 字段);`chg-remove-roles`(2026-07-09)后 `rebind_story_agent` 不再存在;`chg-rename-engine-runner`(2026-07-16)后 `Engine.deps` 字段路径已不存在
- LLM 工具循环有 `MAX_LOOP_STEPS=5` 上限,耗尽时走 `Done(tool_loop_completed, payload={tool_calls_made, last_output})`,**不算** aborted
- `ToolError` / `SkillError` 在 Runner 边界 catch 后产出 `ErrorEvent + Done(aborted)`,不外溢
- `rebind_*` 系列方法是热替换 deps 的唯一接口;任何 duck-typed mutation 已被禁止(2026-07-05 M6 修复);`set_project_root` 通过 `Runner.replace_deps(new_deps)` 整体替换(2026-07-16 前是 `Engine.replace_deps`)
...
- `/大纲` / `/目录` / `/人物` 三个 shipped directives 各自消费 AGENT.md 多行字段(`/大纲` 消费 2 行 题材 + 架构方法;`/目录` 消费 5 行 题材 + 架构方法 + 字数 + 章数 + 分卷;`/人物` 落 `人物/<角色名>.md` 八项固定字段),SKILL.md body 由 LLM 自主判断「已存在 vs 新建 / 追加 vs 覆盖」(per `chg-remove-state-machine-enforcement` 2026-07-12 + shipped skills 2026-07-16)
- Engine 与 Runner 命名分工:`Engine`(会话层)持 `runner: Runner`;`Runner`(单轮派发)是 `Runner.run(ctx)` 公开 API;`engine.loop.run_runner` 不再保留 compat shim(per `chg-rename-engine-runner` 2026-07-16)
