...
| 层级 | 主要范式 | 职责 | 典型组件 |
| --- | --- | --- | --- |
| 前台调度层 | 结构化输出 ReAct(可降级到规则版) | 把用户输入转成 `AgentAction` | `WriterCommandAgent` |
...
- **前台**:结构化输出的轻量 Agent(`WriterCommandAgent`)
...
- 前台 `WriterCommandAgent` 用的是 **Plan-and-Route + 结构化输出**;
