...
> **2026-07-14 重要修订**(覆盖 2026-07-09 修订):本文档原版本以 `writer.runner.loop.run_runner` 为主入口 + `_DefaultRunnerDeps.run_workflow` 占位 stub 为基础。截至 2026-07-13,**`Engine` 类已成为引擎主入口**(`src/writer/engine/engine.py`),`Engine.run(ctx)` 公开 API;`engine.loop.run_runner` 降级为 compat shim(每次构造临时 `Engine(deps, cfg)` 委派给 `engine.run(ctx)`)。
>
> **`Engine` 类形态**(per 2026-07-13 重构):
> - 主类:`Engine` 持 `RunnerDeps`(DI 容器,长生命周期)+ `RunnerConfig`(per-loop 配置)
...
> - `Engine.engine: Engine` 替代旧 `Engine.deps: RunnerDeps`,`__post_init__` 一次性构造
>
> **`RunnerDeps` 当前形态**(2026-07-13 实测):9 字段 + 5 rebind 方法 + 2 普通方法。`tool_loop` / `prose_client` / `review_llm` / `settings` 均存在;`story_agent` 字段已删(`chg-remove-roles`)。
>
...
>
> 本节"已落地的 Engine 层结构"(§5) 描述的是 2026-07-08 形态;2026-07-13 起 `src/writer/engine/` 多出 `engine.py` 主类文件;`RunnerDeps` 字段已扩展到 9 个;其余概念(ReAct / Plan-and-Execute / Tool-Use / Markdown directive)仍生效。
...
| 引擎层 | `match` dispatch + AsyncGenerator | 调度 `AgentAction` 到对应分支,产出事件流 | `writer.runner.loop.run_runner` + `RunnerDeps` |
| 工作流 / 工具 / directive 层 | LangGraph(占位 workflow) + Tool-use + Markdown directive | 长任务、文件 IO、业务规则 | `_DefaultRunnerDeps.run_workflow` + `ToolRegistry` + `DirectiveRegistry` |
...
- **引擎**:`writer.runner.loop.run_runner` AsyncGenerator + `match action.action_type`
  ...
  - `start_workflow` 走 `_DefaultRunnerDeps.run_workflow`(占位,等真实 LangGraph 图)
...
└── loop.py        # run_runner() + _engine_loop() AsyncGenerator
```
...
`run_runner(ctx, deps, *, config)` 返回 `AsyncIterator[Event]`,本身不持有任何会话状态。原因:

1. **流式消费友好**: REPL `async for event` 逐事件渲染,token-level 流式输出可直接接。
2. **取消成本低**: REPL Ctrl+C 可以中断 async iteration,engine 不持有需要清理的资源。
3. **session 可任意 restart**: 重连 session 不需要重置 engine 内存,只需重新调用 `run_runner`。
4. **易测**: 单个 turn = 单次 AsyncGenerator 消费,test 直接 `async for` 就行。

会话级状态(`Engine` 的 turn history / pending_interrupt / project_root 切换 / deps 热替换)属于 `writer.session`,不在 engine 内部。
...
- Engine 包严格 5 文件布局,新增能力(workflow / tool / interrupt / directive)只通过 `RunnerDeps` 扩展,不直接改 `loop.py`
- `RunnerContext / RunnerConfig / Event 子类 / AgentAction` 全程 frozen(`@dataclass(frozen=True)` 或 Pydantic `model_config={"frozen": True}`)
- `run_runner` 是 `AsyncIterator[Event]`,不持有会话状态
- `RunnerDeps` 必须 `@runtime_checkable`,便于测试时 mock
- 6 字段 + 4 方法全部需要在手写 `RunnerDeps` stub 时实现(测试 fakes 经常漏 `tool_loop = None` 字段);`chg-remove-roles`(2026-07-09)后 `rebind_story_agent` 不再存在
