...
>
> **2026-07-16 修订**：本文原版本把 CLI 入口全部写在 `cli/main.py`、列出 `writer outline` 子命令、引用测试基线 ~339。
> 截至 2026-07-16，CLI 已拆分为 `main.py` + `commands.py` + `repl.py` + `_init_backend.py` 四个文件；只剩 `writer doctor` / `writer new <书名>` 两个 Typer 子命令（`writer outline` 等价命令已下线；`writer new --genre` flag 已删除 per `dd229cb`）；测试基线 **549 passing / 1 pre-existing failing**。
> 此外,2026-07-16 `chg-rename-engine-runner` 把状态机主类从 `Engine` 改为 `Runner`(`src/writer/runner/runner.py`),会话层仍叫 `Engine`(`src/writer/session/engine.py`,原 `EngineSession`)。`session.run_turn(text)` 委派给 `session.runner.run(ctx)`,CLI 层接口不变。
...
│       ├── session/           # Engine 跨 turn 状态(长寿命,持 runner)
│       ├── runner/            # Runner 主类 + AsyncGenerator 状态机(短命)
│       │                     # (runner.py + loop.py 工厂函数 + events/context/deps/config)
│       ├── routing/           # IntentRouter Protocol + 3 实现
│       ├── tools/             # Tool 协议 + 9 builtin
│       ├── skills/            # SKILL.md directives (3 shipped: /大纲 /目录 /人物)
...
@app.command()
def new(name: str, dir: str = ".", force: bool = False):
    """创建新书项目。

    截至 2026-07-17(dd229cb):`--genre` flag 已移除。题材选择改在 REPL
    `/init <故事梗概>` 多轮弹窗(`_try_handle_repl_init_brief`)。
    `writer new <书名>` 只创建脚手架,默认题材为 `other`,后续用户
    在 REPL 内通过 `/init` 多选调整。
    """
    workspace = create_new_workspace(name, Path(dir), genres=None, force=force)
...
def _run_repl() -> None:
    ...
    session = Engine(project_root=project_root)
    # Engine.__post_init__ 装配 RunnerDeps 后包装 Runner 实例(session.runner)
    print_welcome()
...
async def _run_runner(user_input: str, session: Engine, console: Console) -> None:
    ...
    async for event in session.run_turn(user_input):
        # session.run_turn 内部: 构造 RunnerContext + 委派给 session.runner.run(ctx)
        # session.runner 是 Runner 实例(2026-07-16 chg-rename-engine-runner 前叫 session.engine)
        match event:
...
- **session / runner / Engine 三层解耦** —— `session: Engine` 持跨 turn 状态(`session_id` / `project_root` / `turns` / `pending_interrupt`);`session.runner: Runner` 持 `RunnerDeps` + `RunnerConfig`;`runner.run(ctx)` 是纯函数式事件流。CLI 不需要 import `RunnerDeps`。
- ~~**`project_root` 反向回填** —— 当 `/init` 走 `answered` 路径创建项目时,payload 里会有 `project_root`,CLI 负责调用 `session.set_project_root(path)` 把新项目绑回去~~(per 2026-07-14 收紧:REPL `/init` 不再创建项目,该流程删除;创建项目的 payload 反向回填逻辑仍保留 `Runner._run_init_command`(2026-07-16 前是 `Engine._run_init_command`),作为 SDK / e2e pipe 等非 REPL 调用方创建项目的兜底通道)。
...
- [05-Runner核心](05-引擎核心.md) —— `Runner.run` 与 Done 分支(原标题「引擎核心」随类重命名)
