...
> **2026-07-13 修订**：REPL `/init <brief>` 多选题材抢先消费已落地——`_try_handle_repl_init_brief` 在 flag 形式分支**之后**、engine 派发**之前**拦截，调 `prompt_genres` + `apply_genre_and_brief`。
...
def handle_repl_input(line: str, session: EngineSession) -> bool:
    ...
    # 2. REPL 抢先消费：/init <brief> 多选题材
    if _try_handle_repl_init_brief(text, session, console):
        return True
