...
| 命令              | 副作用文件                       | 状态变化           |
| ----------------- | -------------------------------- | ------------------ |
| `/init <name>`    | 建项目目录 + 写 `AGENT.md`       | S0 → S1            |
| `/init <brief>`   | 写 `创意/核心创意.md` + 题材行   | 保持 S1            |
| `/大纲`           | 写 `大纲/大纲.md`                | 保持 S1 / → S2     |
| `/目录`           | 写 `大纲/目录.md`                | 保持 S2 / → S3     |
...
### REPL 抢先消费（per 2026-07-13）

REPL `handle_repl_input` 在 brief 形式（无 `--flag`）下**抢先**消费：

```python
def _try_handle_repl_init_brief(text: str, session: EngineSession, console: Console) -> bool:
    """返回 True 表示已消费（不交给 engine）。"""
    if not (text.startswith("/init ") and looks_like_creative_brief(extract_init_brief_text(text))):
        return False
    genres = prompt_genres(console, default=None)  # TTY 交互 / 非 TTY 兜底 ["其他"]
...
- **`should_run_init_brief` + `looks_like_creative_brief` 双闸门**，避免 `/init 我的小说`（建项目）和 `/init 一个穿越到唐朝的程序员`（写 brief）混淆。
- **`looks_like_creative_brief`** 必须识别标点（`。！？；,.!?;`）或长度 > 30 —— 短 token（如 `/init 双生`）走 fallback 到 engine 路径。
