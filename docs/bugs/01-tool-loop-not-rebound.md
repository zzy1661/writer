...
> 修复方式:`Engine.set_project_root()` 检测旧 deps 带 `tool_loop` 时,基于新 `tool_runtime` 重建 `ReActAgent` 并 `rebind_tool_loop(new_loop)` 后再 `Engine.replace_deps(new_deps)` 整体替换;rule-only 部署(`tool_loop=None`)时跳过重建。新增测试 `tests/test_engine_session.py::test_set_project_root_rebinds_tool_loop` 守住契约。
...
| 发现日期 | 2026-07-09 |
| 关联文件 | `src/writer/session/engine.py:94-154`、`src/writer/engine/deps.py:212-243`、`src/writer/engine/deps.py:333-339`(`ReActAgent` 构造) |
...
4. `Engine.set_project_root()` 重建 `tool_runtime = ToolRuntime(project_root=project_b)`,通过 `rebind_tool_runtime(new_runtime)` 把 deps 里的 runtime 替换掉
...
```python
# src/writer/session/engine.py:94-154 (set_project_root)
...
# src/writer/engine/deps.py:184 (RunnerDeps Protocol 字段)
...
```python
# src/writer/engine/deps.py:212-243 (_DefaultRunnerDeps)
...
`RunnerDeps` Protocol 设计时把 `tool_loop` 当作"启动时绑定、运行时不变"的资源(reasoning 见 `engine/deps.py:84` 注释"Forward-referenced as a string to keep the engine package free of direct ``writer.llm.*`` imports"),但 `ReActAgent` 内部硬绑 `_runtime`(因为构造时一次性 `bind_tools(tools)`)。`Engine.set_project_root()` 期望的所有 deps-level 资源都能被替换,但漏了 `tool_loop`。
...
class RunnerDeps(Protocol):
    ...
    def rebind_tool_loop(
        ...
        """Return a new (or in-place mutated) ``RunnerDeps`` with the
        ReAct-style tool loop swapped.

        Called by :meth:`writer.session.Engine.set_project_root`
...
# fix proposal — src/writer/engine/deps.py (_DefaultRunnerDeps 实现)
def rebind_tool_loop(self, new_loop):
    return replace(self, tool_loop=new_loop)

# fix proposal — src/writer/session/engine.py:set_project_root 末尾
...
2. `src/writer/session/engine.py` — `set_project_root` 末尾加重建块
...
# src/writer/session/engine.py
self.deps.tool_loop.rebind_runtime(new_runtime)  # 直接 mutate
```

**否决理由**:
1. `RunnerDeps` 抽象层被绕过,session 层直接修改 deps 内部组件,违反 DI 边界设计(`engine/deps.py:212-216` 注释明示 `dataclasses.replace` 而非 in-place mutation)
...
**否决理由**:为单个调用点(`Engine.set_project_root`)引入工厂注入,过度设计;且 session 调用 production_deps 路径不变,需要在 session 加 factory 字段 → 复杂度高于方案 A。
...
from writer.session.engine import Engine
from writer.tools import ToolRuntime

session = Engine(project_root=Path("/tmp/proj_a"))
...
| `tests/test_engine_deps.py` | `test_production_deps_rebind_tool_loop` | `_DefaultRunnerDeps.rebind_tool_loop(new_loop)` 返回新实例,`tool_loop == new_loop`,其他字段保留 | NEW |
| `tests/test_engine_deps.py` | `test_protocol_duck_typed_rebind_tool_loop` | PlainDeps stub hasattr check,补 `rebind_tool_loop` 后 `isinstance(stub, RunnerDeps)` 通过 | NEW |
...
- **`ReActAgent._settings` 不 rebind**:换项目可能换 `.env` 中的 `OPENAI_API_KEY`(API key 不同)。`_settings` 仍是旧 settings,但 LLM 客户端(LangChain `ChatOpenAI`)是 lazy 构造的,所以**短期**没问题。若未来在 `_settings` 上读取 `temperature` 等字段,会失效。**留给未来的 `RunnerDeps.rebind_settings` 提案**。
- **`prose_client` 同理**:不依赖 project_root,无需 rebind,但其内部 `RealProseClient` 持有 `_llm` 引用,_llm 来自旧 settings。**留给未来**。
- **`RunnerConfig` 与 `cfg` 参数**:`ReActAgent.run(..., cfg: RunnerConfig)` 当前 `del cfg`,未使用。重构时不需关注。
...
- **文档同步**:`备忘 16-Engine与跨轮状态.md` 中 `set_project_root` 段需补一行"tool_loop also rebound"
