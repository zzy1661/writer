...
Covers StoryAgent.draft_outline() branches (fallback):
...
Plus genre Agents (fea-genre-aware-init Block 2):
- HistoryAgent: ≥4 chapters with 史实: / 虚构: markers
- XuanhuanAgent: ≥4 chapters with 境界: markers
- RomanceAgent: 8 ~ 12 chapters with 节拍: markers
"""
...
from writer.roles.story_agent import OutlineResult, StoryAgent
from writer.roles.xuanhuan_agent import XuanhuanAgent


def _make_agent() -> StoryAgent:
    return StoryAgent(Settings())
...
def test_story_agent_draft_outline_with_empty_idea() -> None:
    result = _make_agent().draft_outline("")
...
def test_story_agent_draft_outline_with_whitespace_idea() -> None:
    result = _make_agent().draft_outline("   \n  ")
...
def test_story_agent_draft_outline_truncates_long_title_to_18_chars() -> None:
    long_idea = "穿越到唐朝的程序员发现自己可以使用现代编程语言重构大唐的官僚体系"

    result = _make_agent().draft_outline(long_idea)
...
def test_story_agent_draft_outline_replaces_newlines_in_title() -> None:
    idea_with_newlines = "第一行创意\n第二行细节"

    result = _make_agent().draft_outline(idea_with_newlines)
...
def test_story_agent_returns_four_chapters() -> None:
    result = _make_agent().draft_outline("一个普通的故事创意")
...
def test_story_agent_uses_llm_outline_when_chat_model_is_injected() -> None:
    ...
    result = StoryAgent(Settings(), llm=fake).draft_outline("穿越到唐朝的程序员")
...
def test_story_agent_falls_back_when_llm_outline_is_invalid() -> None:
    fake = _FakeOutlineChat("不是 JSON")

    result = StoryAgent(Settings(), llm=fake).draft_outline("一个普通的故事创意")
...
# ---------------------------------------------------------------------------
# Genre Agents (fea-genre-aware-init Block 2)
# ---------------------------------------------------------------------------


def test_history_agent_draft_outline_emits_shishi_anchors() -> None:
    result = HistoryAgent(Settings()).draft_outline("贞观治世")
...
def test_xuanhuan_agent_draft_outline_emits_jingjie_nodes() -> None:
...
def test_romance_agent_draft_outline_emits_jiepai_beats() -> None:
...
def test_genre_agents_share_outline_result_shape() -> None:
    """All four Agents MUST return the standard OutlineResult shape."""
    for cls in (
        StoryAgent,
        HistoryAgent,
        XuanhuanAgent,
...
def test_genre_agents_inherit_default_fallback_title() -> None:
    ...
    Agent must produce a usable title on empty input.
    """
    for cls in (StoryAgent, HistoryAgent, XuanhuanAgent, RomanceAgent):
